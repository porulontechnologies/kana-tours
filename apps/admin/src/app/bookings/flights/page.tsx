...
function SeatCell({
  ...
  if (seat.occupied) {
    ...
    base += "bg-amber-500 text-white shadow-md scale-110 cursor-pointer";
  } else if (isOther) {
    base += "bg-navy-600 text-white cursor-pointer";
  } else if (seat.type === "window") {
    base += "bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 cursor-pointer";
...
// ─── Sample bookings (displayed in list; replace with API call in production) ──

const SAMPLE_BOOKINGS: FlightBooking[] = [
  {
    id: "FB-2001", pnr: "ABC123", customer: "Amit Patel", email: "amit@email.com",
    airline: "Air India", flightNo: "AI302", from: "Delhi (DEL)", to: "Mumbai (BOM)",
    departureDate: "2024-12-20", departureTime: "08:30", arrivalTime: "10:45",
    passengers: [{ name: "Amit Patel", age: 32, seat: "12A" }],
    class: "Economy", amount: 8200, status: "Confirmed", bookedOn: "2024-12-14",
  },
  {
    id: "FB-2002", pnr: "DEF456", customer: "Kavita Nair", email: "kavita@email.com",
    airline: "IndiGo", flightNo: "6E245", from: "Bangalore (BLR)", to: "Kolkata (CCU)",
    departureDate: "2024-12-22", departureTime: "14:15", arrivalTime: "17:00",
    passengers: [{ name: "Kavita Nair", age: 28, seat: "8C" }],
    class: "Economy", amount: 6500, status: "Confirmed", bookedOn: "2024-12-11",
  },
  {
    id: "FB-2003", pnr: "GHI789", customer: "Suresh Menon", email: "suresh@email.com",
    airline: "Vistara", flightNo: "UK845", from: "Mumbai (BOM)", to: "Chennai (MAA)",
    departureDate: "2024-12-25", departureTime: "10:00", arrivalTime: "12:10",
    passengers: [
      { name: "Suresh Menon", age: 45, seat: "3A" },
      { name: "Lakshmi Menon", age: 42, seat: "3B" },
    ],
    class: "Business", amount: 24000, status: "Confirmed", bookedOn: "2024-12-10",
  },
  {
    id: "FB-2004", pnr: "JKL012", customer: "Neha Kapoor", email: "neha@email.com",
    airline: "SpiceJet", flightNo: "SG142", from: "Delhi (DEL)", to: "Goa (GOI)",
    departureDate: "2024-12-28", departureTime: "06:45", arrivalTime: "09:30",
    passengers: [
      { name: "Neha Kapoor", age: 30, seat: "15D" },
      { name: "Rahul Kapoor", age: 33, seat: "15E" },
    ],
    class: "Economy", amount: 11400, status: "Pending", bookedOn: "2024-12-13",
  },
  {
    id: "FB-2005", pnr: "MNO345", customer: "Arun Joshi", email: "arun@email.com",
    airline: "Air India", flightNo: "AI505", from: "Kolkata (CCU)", to: "Delhi (DEL)",
    departureDate: "2024-12-18", departureTime: "16:30", arrivalTime: "19:00",
    passengers: [{ name: "Arun Joshi", age: 55, seat: "6F" }],
    class: "Economy", amount: 7800, status: "Cancelled", bookedOn: "2024-12-08",
  },
];
...
export default function FlightBookingsPage() {
  ...
  const seatMap = useMemo(() => {
    ...
    return generateSeatMap(selectedFlight.id, flightSearch.travelClass);
  }, [selectedFlight, flightSearch.travelClass]);
  ...
  const rowCols = getRowCols(flightSearch.travelClass);

  const mealOptions = useMemo(() => {
    if (!selectedFlight) return AIRLINE_MEALS.default;
    return getMealOptions(selectedFlight.airline);
  }, [selectedFlight]);
  ...
  const grandTotal = selectedFlight
    ? selectedFlight.price.amount * flightSearch.adults + extrasTotal
    : 0;
  ...
  const handleBook = async () => {
    ...
    try {
      await apiClient.post("/bookings", {
        ...
        contactEmail,
        contactPhone,
        guestCount: flightSearch.adults,
        passengerDetails: {
          flight: selectedFlight,
          class: flightSearch.travelClass,
          agentNotes,
        ...
        externalRef: `${selectedFlight.airline}-${selectedFlight.flightNumber}`,
      });
      setBooked(true);
    } catch {
  ...
  const filteredBookings = SAMPLE_BOOKINGS.filter((b) => {
    const matchesSearch =
      ...
      b.flightNo.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{SAMPLE_BOOKINGS.length} total flight bookings</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredBookings.map((b) => (
                <>
                  <tr key={b.id}>
                    <td className="font-medium text-navy-700">{b.id}</td>
                    <td>
                      <span className="font-mono text-sm bg-gray-100 px-2 py-0.5 rounded">{b.pnr}</span>
                    </td>
                    <td>
                      <div>
                        <p className="font-medium text-navy-800">{b.customer}</p>
                        <p className="text-xs text-gray-400">{b.email}</p>
                      </div>
                    </td>
                    <td>
                      <div>
                        <p className="font-medium text-navy-700">{b.flightNo}</p>
                        <p className="text-xs text-gray-400">{b.airline}</p>
                      </div>
                    </td>
                    <td>
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm">{b.from.split("(")[0].trim()}</span>
                        <Plane size={12} className="text-gray-400" />
                        <span className="text-sm">{b.to.split("(")[0].trim()}</span>
                      </div>
                    </td>
                    <td className="whitespace-nowrap text-gray-500">{b.departureDate}</td>
                    <td className="whitespace-nowrap">
                      <p className="text-sm">{b.departureTime}</p>
                      <p className="text-xs text-gray-400">{b.arrivalTime}</p>
                    </td>
                    <td>
                      <span className={`badge ${b.class === "Business" ? "bg-purple-50 text-purple-700" : "bg-gray-100 text-gray-700"}`}>
                        {b.class}
                      </span>
                    </td>
                    <td className="font-medium">INR {b.amount.toLocaleString()}</td>
                    <td>
                    ...
                    <tr key={`${b.id}-detail`}>
                      <td colSpan={11} className="bg-gray-50 px-6 py-4">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-gray-400">Booked on: {b.bookedOn}</p>
                        </div>
                      </td>
                    </tr>
                  )}
      ...
      {showNewBooking && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
          <div className="my-8 w-full max-w-3xl rounded-2xl bg-white shadow-2xl flex flex-col">
            ...
            <div className="border-b border-gray-100 px-6 py-4 flex-shrink-0">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-navy-800">
                    {selectedFlight ? "Book Flight" : "Search Flights"}
                  </h2>
                  ...
                  {selectedFlight && !booked && (
                    <p className="text-xs font-bold text-amber-600 mt-0.5">
                      Total: INR {grandTotal.toLocaleString("en-IN")}
                      {extrasTotal > 0 && (
                        <span className="ml-1.5 font-normal text-gray-400">
                          (base {(selectedFlight.price.amount * flightSearch.adults).toLocaleString("en-IN")}
                          {` + extras ${extrasTotal.toLocaleString("en-IN")}`})
                        </span>
                      )}
                    </p>
                  )}
              ...
              {/* Step indicator (only when a flight is selected and not yet booked) */}
              {selectedFlight && !booked && (
              ...
              {booked ? (
                <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    Total charged: INR {grandTotal.toLocaleString("en-IN")}
                  </p>
                  <div className="mt-6 flex gap-3">
                    <button onClick={closeNewBooking} className="admin-btn-secondary px-5 py-2.5 text-sm">
                      Close
                    </button>
                    <button
                      onClick={() => {
                ...
                <div className="p-6 space-y-6">
                  ...
                  <form onSubmit={handleFlightSearch} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <AirportSearch
                        ...
                        onChange={(iata, lbl) => {
                          setFlightSearch((s) => ({ ...s, origin: iata }));
                          setOriginLabel(lbl);
                        }}
                      />
                      <div className="relative">
                        <AirportSearch
                          ...
                          onChange={(iata, lbl) => {
                            setFlightSearch((s) => ({ ...s, destination: iata }));
                            setDestinationLabel(lbl);
                          }}
                        />
                        <button
                          type="button"
                          onClick={handleSwap}
                          className="absolute left-1/2 -translate-x-1/2 top-7 z-10 w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 bg-white text-gray-400 hover:border-navy-600 hover:text-navy-600 transition-colors shadow-sm sm:hidden"
                        >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFlightSearch((s) => ({ ...s, departureDate: e.target.value }))}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-navy-700 mb-1.5">Passengers</label>
                        <input
                          type="number"
                          min={1}
                          max={9}
                          value={flightSearch.adults}
                          onChange={(e) => setFlightSearch((s) => ({ ...s, adults: parseInt(e.target.value) || 1 }))}
                          className="admin-input text-center"
                        />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-navy-700 mb-1.5">Travel Class</label>
                        <select
                          value={flightSearch.travelClass}
                          onChange={(e) => setFlightSearch((s) => ({ ...s, travelClass: e.target.value }))}
                    ...
                    >
                      {flightLoading ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />}
                  ...
                  {flightLoading ? (
                    <div className="text-center py-10">
                      <Plane size={40} className="mx-auto text-blue-300 animate-pulse" />
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {sortedFlights.map((flight) => (
                          <div
                            key={flight.id}
                            className="border border-gray-200 rounded-xl p-4 hover:border-navy-300 hover:shadow-sm transition-all"
                          >
                            <div className="grid items-center gap-4 sm:grid-cols-12">
                              {/* Airline */}
                              <div className="sm:col-span-2">
                                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-navy-50 text-xs font-bold text-navy-700">
                                  {flight.airline.slice(0, 2).toUpperCase()}
                                </div>
                                <p className="mt-1 text-sm font-semibold text-navy-800">{flight.airline}</p>
                              ...
                              <div className="sm:col-span-6">
                                <div className="flex items-center justify-between">
                                  <div className="text-center">
                                    <p className="text-xl font-bold text-navy-800">{formatTime(flight.departure.time)}</p>
                                  ...
                                  <div className="flex-1 px-3">
                                    ...
                                    <div className="relative my-1">
                                      ...
                                      <Plane size={12} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-400" />
                                    </div>
                                    <p className="text-center text-[10px] text-gray-400">
                                  ...
                                  <div className="text-center">
                                    <p className="text-xl font-bold text-navy-800">{formatTime(flight.arrival.time)}</p>
                              ...
                              <div className="sm:col-span-1 text-center">
                                <span className="text-[10px] font-medium bg-gray-100 text-gray-600 px-2 py-1 rounded-full whitespace-nowrap">
                                  {flightSearch.travelClass.replace("_", " ")}
                                </span>
                                {flight.seatsAvailable > 0 && (
                                  <p className="mt-1 text-[10px] text-emerald-600 flex items-center justify-center gap-0.5">
                                    <Users size={9} />
                              ...
                              <div className="sm:col-span-3 text-right">
                                ...
                                <p className="text-xl font-bold text-amber-600">
                                  INR {flight.price.amount.toLocaleString("en-IN")}
                                </p>
                                {flightSearch.adults > 1 && (
                                  <p className="text-xs text-gray-500">
                                    Total INR {(flight.price.amount * flightSearch.adults).toLocaleString("en-IN")}
                                  </p>
                                )}
                                <button
                                  onClick={() => openBookingModal(flight)}
                                  className="admin-btn-primary mt-2 flex items-center gap-1.5 text-sm ml-auto px-4 py-1.5"
                                >
                    ...
                    <div className="px-6 py-5 space-y-5">
                      ...
                      <div className="border-t border-gray-100 pt-4">
                        <p className="mb-3 text-sm font-semibold text-navy-800">
                          Passenger Details ({flightSearch.adults} passenger{flightSearch.adults > 1 ? "s" : ""})
                        </p>
                      ...
                      <div className="border-t border-gray-100 pt-4">
                        <label className="block text-xs font-medium text-gray-500 mb-1.5">Agent Notes (optional)</label>
                        <textarea
                          value={agentNotes}
                    ...
                    <div className="px-6 py-5">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1.5 mb-4 text-[10px] text-gray-600">
                        <span className="flex items-center gap-1.5">
                          <span className="inline-block w-4 h-4 rounded bg-blue-50 border border-blue-200" />
                          Window
                        </span>
                        <span className="flex items-center gap-1.5">
                        ...
                        <span className="flex items-center gap-1.5">
                          <span className="inline-block w-4 h-4 rounded bg-amber-500" />
                          Your seat
                        </span>
                        {flightSearch.adults > 1 && (
                          <span className="flex items-center gap-1.5">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                              {BAGGAGE_OPTIONS.map((opt) => (
                                ...
                                >
                                  ...
                                  <p className={`text-xs font-bold mt-1.5 ${opt.price === 0 ? "text-emerald-600" : "text-amber-600"}`}>
                                    {opt.price === 0 ? "Free" : `+INR ${opt.price.toLocaleString("en-IN")}`}
                                  </p>
                                </button>
                              ))}
                            </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                              {mealOptions.map((opt) => (
                                ...
                                >
                                  ...
                                  <p className={`text-xs font-bold mt-1.5 ${opt.price === 0 ? "text-emerald-600" : "text-amber-600"}`}>
                                    {opt.price === 0 ? "Included" : `+INR ${opt.price.toLocaleString("en-IN")}`}
                                  </p>
                                </button>
                              ))}
                            </div>
                      ...
                      <div className="rounded-xl border border-gray-200 p-4">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-[10px] text-gray-400">{flightSearch.adults} pax</p>
                            <p className="font-bold text-navy-800">
                              INR {(selectedFlight!.price.amount * flightSearch.adults).toLocaleString("en-IN")}
                            </p>
                          </div>
                        </div>
                      </div>
                            ...
                            <div key={i} className="px-4 py-3 border-b border-gray-100 last:border-0">
                              ...
                              <div className="mt-2 flex flex-wrap gap-2">
                                <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
                                  Baggage: {bagOpt?.label ?? "—"}
                                  {(bagOpt?.price ?? 0) > 0 && ` (+INR ${bagOpt!.price})`}
                                </span>
                                <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
                                  Meal: {mealOpt?.label ?? "—"}
                                  {(mealOpt?.price ?? 0) > 0 && ` (+INR ${mealOpt!.price})`}
                                </span>
                              </div>
                            </div>
                          );
                      ...
                      <div className="rounded-xl bg-navy-50 p-4 space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">
                            Base fare ({flightSearch.adults} × INR {selectedFlight!.price.amount.toLocaleString("en-IN")})
                          </span>
                        ...
                        {baggageSelections.map((id, i) => {
                          ...
                          return (
                            <div key={i} className="flex justify-between text-sm">
                              ...
                              <span className="font-semibold text-navy-800">
                                INR {opt.price.toLocaleString("en-IN")}
                              </span>
                            </div>
                          );
                        })}
            ...
            {/* Modal footer — only when a flight is selected and not yet booked */}
            {selectedFlight && !booked && (
              <div className="border-t border-gray-100 px-6 py-4 flex items-center justify-between flex-shrink-0">
                <button
                  type="button"
                  onClick={() =>
                    bookingStep > 1 ? setBookingStep((s) => s - 1) : setSelectedFlight(null)
                  }
                  className="admin-btn-secondary px-5 py-2.5 text-sm"
                >
                  {bookingStep > 1 ? "Back" : "Back to results"}
                </button>
