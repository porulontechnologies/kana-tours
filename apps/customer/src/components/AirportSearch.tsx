        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          {suggestions.map((airport, idx) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-navy/5 text-left transition-colors border-b border-gray-50 last:border-0"
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                airport.subType === "AIRPORT" ? "bg-sky/10" : "bg-navy/5"
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-navy">
                  {airport.cityName}
                  <span className="ml-2 text-xs font-bold text-sky bg-sky/10 px-1.5 py-0.5 rounded">
                    {airport.iataCode}
                  </span>
                </p>
